<?php
	...
	if(isset($_POST['btnsave']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO image(userName,userProfession,userPic,size,type) VALUES(:uname, :ujob, :upic,:size,:type)');
			$stmt->bindParam(':uname',$username);
			$stmt->bindParam(':ujob',$userjob);
			$stmt->bindParam(':upic',$userpic);
            $stmt->bindParam(':size',$imgSize);
            $stmt->bindParam(':type',$type);
			
			if($stmt->execute())
			{
				$successMSG = "new record succesfully inserted ...";
				header("refresh:5;imgaddnew.php"); // redirects image view page after 5 seconds.
			}
			else
			{
